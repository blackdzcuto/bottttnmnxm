...
module.exports.run = async ({ api, event, args, getText }) => {
  ...
  if (args.join() == "") {api.sendMessage("📣Bạn có thể dùng snoti kèm\n1: png\n2: jpg\n3: mp3\n4: mp4",event.threadID, event.messageID);
    }
  if (args[0] == "1") {
...
var abc = event.messageReply.attachments[0].url;
  ...
  for (const idThread of allThread) {
    if (isNaN(parseInt(idThread)) || idThread == event.threadID) ""
      ...
      api.sendMessage({body:"📣 𝙏𝙝𝙤̂𝙣𝙜 𝘽𝙖́𝙤 𝘽𝙮 𝘼𝘿𝙈𝙄𝙉 🌺" + args.join(` `),attachment: fs.createReadStream(path) }, idThread, (error, info) => {
  ...
  for (const idThread of allThread) {
    if (isNaN(parseInt(idThread)) || idThread == event.threadID) ""
      ...
      api.sendMessage("📣 𝙏𝙝𝙤̂𝙣𝙜 𝘽𝙖́𝙤 𝘽𝙮 𝘼𝘿𝙈𝙄𝙉 🌺\n\n" + args.join(` `), idThread, (error, info) => {
        if (error) cantSend.push(idThread);
      });
      count++;
      await new Promise(resolve => setTimeout(resolve, 500));
